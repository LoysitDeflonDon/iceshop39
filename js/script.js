...
const CATEGORY_BINS = {
    ...
    "Картриджи":  "6a1c63ad21f9ee59d2a1ac82",
    "Одноразки":  "6a1c639721f9ee59d2a1ac4f"
};
...
function setCachedCategory(catName, data) {
    try {
        localStorage.setItem(getCacheKey(catName), JSON.stringify(data));
        localStorage.setItem(getCacheTimeKey(catName), Date.now().toString());
    } catch(e) {}
}
...
async function loadCategoryFromBin(catName, binId) {
    ...
    console.warn(`⚠️ ${catName}: загружен кеш (${cached ? cached.length : 0} товаров)`);
    return cached || [];
}
    ...
    container.innerHTML = items.map(item => {
        ...
        if (item.flavors?.length) {
            return `<div class="product-card" data-id="${item.id}" data-has-flavors="true">
                <img class="product-image" src="${itemImage}" loading="lazy" decoding="async" onerror="this.src='https://placehold.co/400x300/1E293B/3B82F6?text=No+Image'">
